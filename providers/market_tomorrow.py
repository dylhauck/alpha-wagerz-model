...
def local_slate_window():
    ...
    local_tomorrow = datetime.now(tz).date() + timedelta(days=1)
    local_start = datetime.combine(
        local_tomorrow,
        time.min,
        tzinfo=tz,
...
def fetch_tomorrow_events():
    start, end = tomorrow_event_window()
...
def save_cache(output):
    ...
    tomorrow = (
        datetime.now(ZoneInfo(MARKET_TIMEZONE)).date()
        + timedelta(days=1)
    ).isoformat()
    save_json(output, CACHE_DIR / f"market_lines_{tomorrow}.json")


def build_tomorrow_market_lines():
    ...
    try:
        api_events = fetch_tomorrow_events()
        print(f"📅 Odds-API.io returned {len(api_events)} pending MLB events tomorrow")
